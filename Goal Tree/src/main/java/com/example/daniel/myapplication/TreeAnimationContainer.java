...
public class TreeAnimationContainer {
    ...
    private TreeAnimationContainer(ImageView imageView) {
        init(imageView);
    }

    private static TreeAnimationContainer sInstance;

     static TreeAnimationContainer getInstance(ImageView imageView) {
        if (sInstance == null)
            sInstance = new TreeAnimationContainer(imageView);
    ...
    private AnimationFrame getNext() {
        mIndex++;
        if (mIndex >= 301 &&!growthDone) {
            growthDone = true;
            newDuration = true;
            if (mIndex>mAnimationFrames.size()) mIndex=mAnimationFrames.size()-2;
        }
        if (mIndex>=mAnimationFrames.size() && newDuration){
            mIndex = 301;
        }
        else if (mIndex>=mAnimationFrames.size()) {mIndex=mAnimationFrames.size()-2;}
